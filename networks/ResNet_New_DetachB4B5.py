...
class ResNet(nn.Module):
    ...
    def detach_Freq(self, out, stop_AM, stop_PH):
        ...
        if stop_PH:
            im_PH = im_PH.detach()
        ep = (torch.exp((1j)*im_PH)).cuda()
        out = torch.fft.ifftn(im_AM*(ep)).float()
        return out
    ...
    def forward(self, x, lin=0, lout=5):
        ...
        if lin < 1 and lout > -1:
            ...
            out = self.bn1(out)
            out = F.relu(out)
            #out = self.detach_Freq(out, stop_AM, stop_PH) #Down Saturn1
        if lin < 2 and lout > 0:
            out = self.layer1(out)
            #out = self.detach_Freq(out, stop_AM, stop_PH) #Up Saturn 8
        if lin < 3 and lout > 1:
            out = self.layer2(out)
            #out = self.detach_Freq(out, stop_AM, stop_PH) #Down Saturn 8
        if lin < 4 and lout > 2:
            out = self.layer3(out)
            if stop_AM:
        ...
        if lin < 5 and lout > 3:
            out = self.layer4(out)
            #out = self.detach_Freq(out, stop_AM, stop_PH) #Up Saturn 1
        if lout > 4:
